...
*Prepared by: Researcher agent | Date: 2026-03-28 | Cycle 3*

> **Scope note:** This cycle's research builds directly on the Cycle 2 report. The Cycle 2 plan (16 commits) remains unimplemented — the seven plan-review failures (GH#1–GH#7) were never resolved. This report provides updated intelligence specifically targeted at unblocking implementation, correcting outdated information, and resolving the known pre-implementation issues.

---

## Summary

The Cycle 2 implementation plan has a missing prerequisite that explains why no code has been committed across two full cycles: `server/src/claude.ts` spawns the Claude Code CLI as a subprocess (`claude --print`). The CLI cannot accept a `tools` parameter, return `usage.input_tokens`, or expose `thinking_blocks`. Every Cycle 2 feature that depends on these — structured tool use (Story 14), token budget tracking (Story 9), thought log (Story 3) — is architecturally impossible until `claude.ts` is migrated to `@anthropic-ai/sdk`. This migration should be **Commit 0** (before the schema changes). Beyond this blocker, three other Cycle 2 assumptions need updating: `git-diff-view` should be replaced with `react-diff-view` (confirmed ESM support via `/esm` subpath, resolves GH#3), the `diff` npm package v8.0.4 ships full TypeScript types and covers both `createPatch()` and `applyPatch()` (use it; no inline LCS needed), and `@temporalio/worker` v1.14.0 does **not** run on Bun (Node.js-only — use `Promise.all()` + native `phase_states` crash recovery instead). The competitive landscape has not materially shifted; Mastra has matured to v1.4.0 with confirmed TypeScript-native agent memory support.
...
| **MetaGPT** | 62K+ stars, Python. Structured document communication model. | No meaningful update. Pattern reference only. |
...
| **AutoGen** (Microsoft) | Multi-agent conversation framework. Merging with Semantic Kernel. | ❌ Avoid — entering maintenance mode. No TypeScript. No update. |
